...
/*
 * dw-mipi-dsi driver ported from a Rockchip Linux 4.4 to work with a 5.x and newer kernels
 * 
 * Copyright (C) Rockchip Electronics Co.Ltd
 * Author:
 *      Guochun Huang <hero.huang@rock-chips.com>
 *      Norbert Kiszka <linux@elektrykplakal.pl>
 */

// TODO: dsc compression?
...
#define DRIVER_NAME    "dw-mipi-dsi-orangerigol"
...
#define M3_VID_MODE_TYPE_BURST_SYNC_PULSES	0x0
#define M3_VID_MODE_TYPE_BURST_SYNC_EVENTS	0x1
#define M3_VID_MODE_TYPE_BURST		0x2
...
struct dw_mipi_dsi {
	...
	// Norbert: all members below was added from 5.x kernels
...
static void dw_mipi_dsi_video_mode_config(struct dw_mipi_dsi *dsi)
{
	...
	if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_BURST)
		val |= M3_VID_MODE_TYPE_BURST;
	else if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_SYNC_PULSE)
		val |= M3_VID_MODE_TYPE_BURST_SYNC_PULSES;
	else
		val |= M3_VID_MODE_TYPE_BURST_SYNC_EVENTS;
...
static int mipi_dphy_power_on(struct dw_mipi_dsi *dsi)
{
	...
	if (dsi->dphy.phy) {
		ret = phy_set_mode(dsi->dphy.phy, PHY_MODE_MIPI_DPHY);
		if (ret) {
				 ...
				 struct drm_connector_state *conn_state)
{
	...
	switch (dsi->format) {
		...
		case MIPI_DSI_FMT_RGB565:
			...
			break;
		default:
			DRM_DEV_ERROR_RATELIMITED(dsi->dev, "Bad dsi->format %u", dsi->format);
			return -EINVAL;
	}
	...
	if (dsi->dual_connector_split) {
		...
		if (dsi->left_display)
			s->output_if_left_panel |= dsi->id ?
						   VOP_OUTPUT_IF_MIPI1 :
						   VOP_OUTPUT_IF_MIPI0;
	}
	
	/* dual link dsi for rk3399 */
	if (IS_DSI1(dsi) && dsi->pdata->soc_type == RK3399)
		s->output_flags |= ROCKCHIP_OUTPUT_DATA_SWAP;

	if (!(dsi->mode_flags & MIPI_DSI_MODE_VIDEO)) {
		s->output_flags |= ROCKCHIP_OUTPUT_MIPI_DS_MODE;
		// Norbert: I can't find any documentation about this soft_te irq handler. It's from Rockchip kernel 5.10, not from 4.4
...
static int dw_mipi_dsi_connector_get_modes(struct drm_connector *connector)
{
	...
	if (dsi->bridge && (dsi->bridge->ops & DRM_BRIDGE_OP_MODES))
...
dw_mipi_dsi_connector_detect(struct drm_connector *connector, bool force)
{
	...
	if (dsi->bridge && (dsi->bridge->ops & DRM_BRIDGE_OP_DETECT))
		return drm_bridge_detect(dsi->bridge);
				      ...
				      struct dw_mipi_dsi *dsi)
{
	...
	if (dsi->bridge) {
		...
		flags = dsi->bridge->ops & DRM_BRIDGE_OP_MODES ?
			DRM_BRIDGE_ATTACH_NO_CONNECTOR : 0;
		ret = drm_bridge_attach(encoder, dsi->bridge, NULL, flags);
		if (ret) {
	...
	/* Otherwise create our own connector and attach to a panel */
		...
		ret = drm_connector_attach_encoder(&dsi->connector, encoder);
		if (ret < 0) {
...
// Norbert: temporary workaround with probing panel
extern struct drm_panel *panel_last_probed;
			     ...
			     void *data)
{
	...
	dsi->panel = panel_last_probed; // Norbert: temporary workaround with probing panel
	if (!dsi->panel) {
		dsi->bridge = of_drm_find_bridge(dsi->client);
		if (!dsi->bridge) {
			DRM_DEV_ERROR(dsi->dev, "Can't find panel or bridge");
			return -EPROBE_DEFER;
		}
	}
				   ...
				   struct mipi_dsi_device *device)
{
	...
	{
		DRM_DEV_ERROR(dsi->dev, "Bad lanes number: %u\n", device->lanes);
		return -EINVAL;
	}
	...
	//return component_add(host->dev, &dw_mipi_dsi_ops);
	return 0;
}
...
static int dw_mipi_dsi_probe(struct platform_device *pdev)
{
	...
	if (device_property_read_bool(dev, "dual-connector-split")) {
		dsi->dual_connector_split = true;

		if (device_property_read_bool(dev, "left-display"))
			dsi->left_display = true;
	}
	
	if (device_property_read_u32(dev, "split-area", &dsi->split_area))
		dsi->split_area = 0;
	...
	if (ret)
	{
		dev_err(dev, "Failed to attach dphy: %d\n", ret);
		return ret;
	}
	...
	return component_add(dev, &dw_mipi_dsi_ops);
}
...
static const struct of_device_id dw_mipi_dsi_dt_ids[] = {
	{ .compatible = "rockchip-orangerigol,px30-mipi-dsi", .data = &px30_socdata, },
	{ .compatible = "rockchip-orangerigol,rk1808-mipi-dsi", .data = &rk1808_socdata, },
	{ .compatible = "rockchip-orangerigol,rk3128-mipi-dsi", .data = &rk3128_socdata, },
	{ .compatible = "rockchip-orangerigol,rk3288-mipi-dsi", .data = &rk3288_socdata, },
	{ .compatible = "rockchip-orangerigol,rk3366-mipi-dsi", .data = &rk3366_socdata, },
	{ .compatible = "rockchip-orangerigol,rk3368-mipi-dsi", .data = &rk3368_socdata, },
	{ .compatible = "rockchip-orangerigol,rk3399-mipi-dsi", .data = &rk3399_socdata, },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, dw_mipi_dsi_dt_ids);

struct platform_driver dw_mipi_dsi_rockchip_orangerigol_driver = {
